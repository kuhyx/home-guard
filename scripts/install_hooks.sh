...
#      .pre-commit-config.yaml (ruff, file-length, dependency freshness, the
#      crdt_sync override check, ...) plus the pre-push CI mirror.
...
# rejected commit from leaving a stray bump staged for the next one. It also
# means a manual `pre-commit run` no longer bumps: as an always_run hook the
# bump fired on every gate rerun and walked the app from 1.0.0 to 1.0.10 in
# one afternoon without a single release in between.
# ============================================================================
...
# The Flutter app lives in app/, so point the bump at its pubspec.
exec bash "$ROOT/scripts/bump_patch_version.sh" --pubspec app/pubspec.yaml
HOOK_BODY
    chmod 755 "$HOOK"
}
...
main() {
    ...
    install_wrapper
    echo "Installed pre-commit hooks: $HOOK (+ pre-push)"
}
